...
# Encoder: Compresses data
        self.encoder = nn.Sequential(
            ...
            nn.ReLU(),       # LeakyReLU prevents "dead neurons"
...
def global_evaluate(server_round: int, arrays: ArrayRecord) -> MetricRecord:
    ...
    _,_,X_test_full, X_test_validation, y_true,X_train_classifier,y_class = load_centralized_dataset(which_dataset= 1)
    ...
    tn_90, fp_90, fn_90, tp_90 = confusion_matrix(y_true, y_pred_90).ravel()
    ...
    f1_90 = f1_score(y_true, y_pred_90)
    
    fdr_dt = fp_dt / (fp_dt + tp_dt) if (fp_dt + tp_dt) > 0 else 0
    fdr_90 = fp_90 / (fp_90 + tp_90) if (fp_90 + tp_90) > 0 else 0
    print(f"F1-Score DT: {f1_dt:.4f}",f"F1-Score 90: {f1_90:.4f}")
    print(f"False Discovery Rate DT: {fdr_dt:.4f}", f"False Discovery Rate 90: {fdr_90:.4f}")
    ...
    return MetricRecord({
      ...
      #  "threshold tr_star": float(tr_star),#"classification-report": classreport,
        ...
        "F1-Score 90": float("{:.4f}".format(f1_90)),
        "FDR DT": float("{:.4f}".format(fdr_dt)),
        "FDR 90": float("{:.4f}".format(fdr_90)),                
